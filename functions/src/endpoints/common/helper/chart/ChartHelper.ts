...
class ChartHelper {
    ...
    public static async getShowEpisodesChart(showId?: number, seasons?: SeasonResponse[]) {
        const body = await Promise.all(
            seasons ? seasons.map(async (season) => ChartHelper.getSeason(showId, season.season_number)) : []
        );
        return ({
            ...
            chart: {
                ...
                body: {
                    episode: ChartHelper.getEpisodeColumn(body),
    ...
    static async getSeason(showId?: number, seasonNumber?: number): Promise<ChartBodyCellProps[]> {
        ...
        return season.episodes ? season.episodes.map(ChartHelper.getEpisodeCell) : [];
    }

    static getEpisodeCell = (episode: Episode) => {
        const rating = ChartHelper.getRating(episode?.vote_average);
        return {
            rating,
    ...
    static getEpisodeColumn = (body: any[][]) => Array(ChartHelper.getSeasonWithMaxEpisodes(body)).fill('').map((_, i) => i + 1);
